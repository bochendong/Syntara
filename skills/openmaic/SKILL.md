...
- Do not assume the OpenClaw agent's own model or API key will be reused by Syntara.
- Syntara classroom generation uses Syntara server-side provider config.
- This skill must not rely on any request-time model or provider overrides.
- Only Syntara server-side config files may control provider selection and defaults.
...
Use this when the user has not installed Syntara yet or when you need to confirm which local checkout to use.
...
### 4. Start And Verify Syntara

After the user has chosen a startup mode and configured keys, start Syntara using the chosen method, then verify the service with `GET {url}/api/health`.
