...
import { getHtmlLessonTestHeaders } from '../_lib/api';
import {
  buildErrorResult,
...
import { evaluatePreview, getPreviewStatus } from '../_lib/preview';
import { readSavedState, writeSavedState } from '../_lib/storage';
import {
...
export default function GenerationHtmlOpenMaicLessonTestPage() {
  ...
  const [selectedTier, setSelectedTier] = useState<PageCountTier>('under20');
  ...
  useEffect(() => {
    ...
    void readSavedState()
      .then((saved) => {
        ...
        setSelectedTier(saved.selectedTier || 'under20');
  ...
  const generatePlan = useCallback(async (): Promise<LessonPlanResult | null> => {
    ...
    setIsPlanning(true);
    setRunMessage('正在用 OpenMAIC 思路规划整节课大纲，再交给 HTML 链路...');
    ...
    try {
      const response = await backendFetch('/api/generation-quality/html-openmaic-lesson-plan', {
        method: 'POST',
      ...
      if (!response.ok || data.success === false || !data.plan?.slides?.length) {
        setPlanErrorsByKey((previous) => ({
          ...
          [key]: buildErrorResult(
            data,
            response.status,
            `OpenMAIC 思路整课规划失败：HTTP ${response.status}`,
          ),
        }));
        return null;
      }
          ...
          <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
            <div>
              <div className="flex items-center gap-2 text-sm font-semibold text-blue-700">
                ...
                OpenMAIC + HTML Deck QA
              </div>
              <h1 className="mt-3 text-3xl font-bold tracking-normal text-slate-950">
                OpenMAIC 思路 HTML 整课生成测试
              </h1>
              <p className="mt-2 max-w-4xl text-sm leading-6 text-slate-600">
                新规划器先复刻 OpenMAIC 的 whole-lesson
                思路，生成整课教学顺序、互动/测验节点、页面目标和每页容量预算；
                再把这些结构化大纲交给旧 HTML 单页生成器。旧 HTML 测试页和旧 planner 都不改。
              </p>
            </div>
          ...
          <aside className="flex flex-col gap-4 xl:sticky xl:top-6 xl:max-h-[calc(100vh-3rem)]">
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <p className="mt-1 text-xs leading-5 text-slate-500">
                选择 testfile 文件和页数档位，先让 AI 分配页面容量。PDF 整课默认用 20
                页以下，避免单页过载。
              </p>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              {currentSlide ? (
                <div className="mt-4 grid gap-3 text-sm lg:grid-cols-[1.2fr_0.8fr]">
                  ...
                  <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                    <div className="text-xs font-semibold text-slate-500">知识参考范围</div>
