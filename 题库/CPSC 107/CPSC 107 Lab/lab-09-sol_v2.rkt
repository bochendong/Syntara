...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname lab-09-sol_v2) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
(require spd/tags)
...
(define (can-encode? ht s)
  (cond
    [(string? ht) (string=? ht s)]
    [else
     (or (can-encode? (htree-zero ht) s)
...
(define (count-odds pt)
  (local
    [(define (fn-for-pt pt l)
       (cond
         [(and (pt-f? pt) (odd? l))
          (+ 1 (fn-for-lopt (pt-subs pt) ( + 1 l)))]
         [else (fn-for-lopt (pt-subs pt) ( + 1 l))]
       ))

     (define (fn-for-lopt lopt l)
       (cond
         [(empty? lopt) 0]
         [else
          (+ (fn-for-pt (first lopt) l)
             (fn-for-lopt (rest lopt) l)
             )
          ]
       )
       )
...
(check-expect (encode HTMAGIC "X") false)
(check-expect (encode "A" "A") empty)
(check-expect (encode HTMAGIC "A") (list 0))
(check-expect (encode HTMAGIC "B") (list 1 0 0))
(check-expect (encode HTMAGIC "C") (list 1 0 1 0))
(check-expect (encode HTMAGIC "D") (list 1 0 1 1))
(check-expect (encode HTMAGIC "R") (list 1 1))

(@template-origin HTree accumulator try-catch)

(define (encode ht s)
  (local
    [(define (encode-acc ht acc)
       (cond
         [(string? ht) (if (string=? ht s)
                           acc
                           false)]
         [else
          (local
            [(define left-result (encode-acc (htree-zero ht)(append acc (list 0))))
             (define right-result (encode-acc (htree-one ht)(append acc (list 1))))]

            (cond
              [(not (false? left-result)) left-result]
              [(not (false? right-result)) right-result]
              [else false]
            ))
          ]
       ))]
...
(define (encode ht s)
  (local
    [(define (encode ht rsf)
       (cond
         [(string? ht) (if (string=? ht s) rsf false)]
         [else
          (local
            [(define try (encode (htree-zero ht)
                                 (append rsf (list 0))))]
            (if (not (false? try))
                try
                (local
                  [(define try (encode (htree-one ht)
                                       (append rsf (list 1))))]
                  (if (not (false? try))
                      try
                      false
