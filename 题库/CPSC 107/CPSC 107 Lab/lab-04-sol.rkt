...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname lab-04-sol) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
(require spd/tags)
(require 2htdp/image)
...
(define (sink lob)
  (cond
    [(or (empty? lob) (empty? (rest lob))) lob]
    [else
     (cond
       [(and (not (empty? (sink (rest lob))))
             (bubble? (first (sink (rest lob)))))
