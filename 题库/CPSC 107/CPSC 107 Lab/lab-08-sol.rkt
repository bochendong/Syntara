...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname lab-08-sol) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
(require spd/tags)
...
(check-expect (solve (make-chst (list M R) (list N O P S V))) (list S P O M R))
(check-expect (solve (make-chst empty (list M N O P R S V))) (list S R P O M))


(@template-origin encapsulated ChoiceState (listof ChoiceState) try-catch)
(define (solve c0)
  (local
    [
     (define (solve-c c)
       (cond
         [(all-crits? (chst-picked c)) (chst-picked c)]
         [(empty? (chst-options c)) false]
         [else
          (solve-loc (neighbour-funciton c))
          ]
       ))

     (define (solve-loc loc)
       (cond
         [(empty? loc) false]
         [else
          (local
            [(define try (solve-c (first loc)))]
            (if (not (false? try))
                try
                (solve-loc (rest loc))))
          ]))

     (define (neighbour-funciton c)
       (local
         [(define picked (chst-picked c))
          (define options (chst-options c))
          ]
         (list
          (make-chst (cons (first options) picked)    ; 带 options 中的第一个小丑
                     (rest options))
          (make-chst picked (rest options))          ; 不带 options 中的第一个小丑
          )))
     ]
    (solve-c c0)
    )
