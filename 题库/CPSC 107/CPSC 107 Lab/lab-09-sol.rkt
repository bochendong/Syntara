...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname lab-09-sol) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
(require spd/tags)
...
(@template-origin HTree try-catch)
...
(define (count-odds pt)
  (local [(define (fn-for-pt pt l)
            (if (and (pt-f? pt) (odd? l))
                (add1 (fn-for-lopt (pt-subs pt)
                                   (add1 l)))
                (fn-for-lopt (pt-subs pt)
                             (add1 l))))
