...
(define (find-at-depth the-map start depth)  
  (local
    [(define (neighbour-function node)
       (map (lambda (x) (generate-node the-map x))
            (node-nexts node))
       )

     (define (find-path-node node path)
       (local
         [(define number (node-number node))
          (define neighbours (neighbour-function node))
          ]
         (cond
           [(= depth (length path)) number]
           [(member? number path) false]
           [else
            (find-path-los neighbours (append path (list number)))])))

     (define (find-path-los los path)
       (cond
         [(empty? los) false]
         [else
          (local
            [(define try (find-path-node (first los) path))]
            (if (not (false? try))
                try
                (find-path-los (rest los) path)
            ))
          ]
       ))
     ]
    (find-path-node (generate-node the-map start) empty)
    )
  )
