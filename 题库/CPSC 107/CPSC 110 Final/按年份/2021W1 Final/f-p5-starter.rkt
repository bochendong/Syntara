...
find-easiest-increasing-path-tr function. This is by far the most difficult
problem in the exam. We expect it to be worth no more than 10%. Budget your
time accordingly considering both its difficulty and weight.
...
steps than the previous stairs.  If there is no such path, it produces false.
Otherwise, it finds the path with the fewest number of steps, and for that
path it produces a list of the Stairs labels on the path starting with the first
Stairs and ending with the last Stairs.
