...
;(define (find-path/gas start dest tank-size map) false) ;stub

(@template-origin encapsulated City (listof Road) Road accumulator try-catch)

(define (find-path/gas start dest tank-size map)
  (local [(define (fn-for-city c gas cpath rpath)
            (cond [(member?  (city-name c) cpath)  false]
                  [(string=? (city-name c) dest)   (reverse rpath)]
                  [else
                   (fn-for-lor (city-roads c)
                               (min tank-size (+ gas (city-gas c)))
                               (cons (city-name c) cpath)
                               rpath)]))

          (define (fn-for-lor lor gas cpath rpath)
            (cond [(empty? lor) false]
                  [else
                   (local [(define try
                             (fn-for-road (first lor) gas cpath rpath))]
                     (if (not (false? try))
                         try
                         (fn-for-lor (rest lor) gas cpath rpath)))]))

          (define (fn-for-road r gas cpath rpath)
            (if (< gas (road-distance r))
                false
                (fn-for-city (generate-city (road-to r) map)
                             (- gas (road-distance r))
                             cpath
    ...
    (fn-for-city (generate-city start map) 0 empty empty)))
