...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname 2020w1-p2-solution) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
(require spd/tags)
(require 2htdp/image)
...
(check-expect (longest-sequence-length (list)) 0)
(check-expect (longest-sequence-length (list 9)) 1)
(check-expect (longest-sequence-length (list 3 4)) 2)
(check-expect (longest-sequence-length (list 4 3)) 1)
(check-expect (longest-sequence-length (list 8 9 1 8 6 7)) 2)
(check-expect (longest-sequence-length (list 1 2 3 4 5 6 7)) 7)
(check-expect (longest-sequence-length (list 8 9 1 8 6 7 8)) 3)
(check-expect (longest-sequence-length (list 6 7 8 8 9 1 8)) 3)
(check-expect (longest-sequence-length (list 8 7 8 -3 -2 -1 5)) 3)
(check-expect (longest-sequence-length (list 7 5 3 -1)) 1)

(define (longest-sequence-length lon0)
  (local [(define (fn-for-lon lon prev curr rsf)
            (cond [(empty? lon) rsf]
                  [else
                   (if (< prev (first lon))
                       (fn-for-lon (rest lon) (first lon) (+ 1 curr) (max (+ 1 curr) rsf))
                       (fn-for-lon (rest lon) (first lon) 1 (max curr rsf)))
                   ]))
            ]
    (fn-for-lon lon0 -1 0 0)))
