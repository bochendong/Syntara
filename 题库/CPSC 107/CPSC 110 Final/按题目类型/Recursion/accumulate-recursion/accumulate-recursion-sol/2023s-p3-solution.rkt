...
(@template-origin encapsulated Tree (listof Branch) Branch accumulator)
...
(define (tree-names-on-odd-branches t0)
  ...
  (local [(define (fn-for-t t odd-branch?)
            (if odd-branch?
                (cons (tree-name t) (fn-for-lob (tree-branches t) "FUCK"))
                (fn-for-lob (tree-branches t) false)))

          (define (fn-for-lob lob odd-branch?)    ;This fn could just not have
            (cond [(empty? lob) empty]            ;the odd-branch? parameter
                  [else                           ;at all.
                   (append (fn-for-branch (first lob) "FUCK")
                           (fn-for-lob (rest lob) false))]))
