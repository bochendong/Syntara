...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname 2020w1-p3-starter) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
(require spd/tags)
...
(define visited-values
  (list empty                             ;at 1
        (list (list "R" "S"))             ;at 3 when fn-for-t is called with V
        (list "R" "S" "V" "W")            ;at 5 when fn-for-t is called with T
        (list "R" "S" "V" "W" "T" "X")))  ;at 7 when fn-for-t is called with U

(define tree-wl-values
  (list empty                                       ;at 1
        (list W T U)                                ;at 3 when fn-for-t is called with V
        (list U)                                    ;at 5 when fn-for-t is called with T
        (list)))                                    ;at 7 when fn-for-t is called with U

(define path-wl-values
  (list empty                                       ;at 1
        (list (list "R" "S") (list "R") (list "R")) ;at 3 when fn-for-t is called with V
        (list (list "R"))                           ;at 5 when fn-for-t is called with T
        (list)))                                    ;at 7 when fn-for-t is called with U
