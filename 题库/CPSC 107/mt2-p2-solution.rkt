...
(check-expect (song-by-key-and-danceability S0 "C major" 50) false)

(check-expect (song-by-key-and-danceability S13 "A-flat major" 90) S8)
(check-expect (song-by-key-and-danceability S13 "A-flat major" 89) S8)
...
(check-expect (song-by-key-and-danceability S13 "A major" 77) S5)
(check-expect (song-by-key-and-danceability S13 "G major" 30) S3)
(check-expect (song-by-key-and-danceability S13 "G-flat major" 30) false)
(check-expect (song-by-key-and-danceability S13 "C major" 70) S6)
(check-expect (song-by-key-and-danceability S13 "C major" 86) false)

;; *** MUST NOT EDIT ANY LINE ABOVE HERE except cwl tag ***
                          
(@template-origin encapsulated Playlist ListOfSong Song try-catch)

(define (song-by-key-and-danceability s0 key drating)
  (local [(define (fn-for-song s)
            (if (string=? (song-key s) key)
                s
                (fn-for-playlist (song-playlist s))))
          ...
          (define (fn-for-los los)
            (cond [(empty? los) false]
                  [else
                   (local [(define try (fn-for-song (first los)))]
                     (if (not (false? try))
                         try
                         (fn-for-los (rest los))))]))

          (define (fn-for-playlist p)
            (if (>= (playlist-danceability p) drating)
                (fn-for-los (playlist-songs p)) 
                false))]
