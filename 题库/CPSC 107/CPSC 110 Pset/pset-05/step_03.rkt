...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname step_03) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
...
;; =render-game============render-game=======render-game=======render-game=
...
(@template
 (define (next-game g)
   (make-game (remove-caught (next-balls ...) ...) ...)))

;; Your on-tick function MUST be designed as a composition of two other
;;       functions called game-with-next-balls and game-with-caught-balls.

;; 其实就是把所有跟g相关的移除掉

(check-expect(next-game(make-game
(list(make-ball(/ WIDTH 2)60 1 2)
(make-ball(+ 10(/ WIDTH 2))30 1 5))
(/ WIDTH 2)))
(make-game(list(make-ball(+ 10(/ WIDTH 2))30 1 5))
(/ WIDTH 2)))

(define (next-game g)
  (make-game (remove-caught (next-balls (game-balls g)) (game-paddle g))
             (game-paddle g)))


;; =remove-caught============remove-caught=======remove-caught=======remo
...
(@template
 (define (remove-caught lob)
   (cond [(empty? lob) (...)]
         [else
          (... (fn-for-ball (first lob))
               (remove-caught (rest lob)))])))


(define (remove-caught lob p)
  (cond [(empty? lob) lob]
        [else
         (if (touch-paddle? (first lob) p)
             (remove-caught (rest lob) p)
             (cons (first lob) (remove-caught (rest lob) p)))]))
...
;; =handle-mouse============handle-mouse=======handle-mouse=======handle-m
...
(define (handle-click g x y me)
    (cond [(mouse=? me "button-down")
           (make-game
            (cons (make-ball x y (- 5 (random 11)) (- 5 (random 11))) (game-balls g))
            (game-paddle g)
            )]
        [else g])
)
...
;; =====handle-key============handle-key=======handle-key=======handle-key

(@htdf handle-key)
(@signature Game KeyEvent -> ListOfBall)
