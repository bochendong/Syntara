...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname Draft) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
...
(define (fn-for-lod lob)
  (cond [(empty? lob) (...)]
        [else
         (... (fn-for-dot (first lob))   ; Natural helper
              (fn-for-lod (rest lob)))   ; Natural Recusion
        ]))
...
(define (render-balls lob)
  (cond [(empty? ws) (...)]                ;BASE CASE
        [else
         (... (fn-for-ball (first lob))       ; Natural helper
              (render-balls (rest lob)))   ; Natural Recusion
        ]))
)
(define (render-balls lob)
  (cond
...
(define (next-balls lob)
  (cond [(empty? lob) (...)]                ;BASE CASE
        [else
         (... (fn-for-ball (first lob))        ; Natural helper
              (next-balls (rest lob)))         ; Natural Recusion
        ]))
)
