...
export function getActionDescriptions(allowedActions: string[]): string {
  const descriptions: Record<string, string> = {
    ...
    'practice.propose_generation':
      'Propose selecting or generating practice questions. Use when the learner explicitly asks for exercises, a targeted review set, quiz, or problem-bank selection. Prefer source="problem_bank" for strict bank matches. When the bank has no usable matches or no active questions, use source="generated_from_course" with persistToProblemBank=true so the question-generation agent creates course-grounded questions and saves them before practice. Only use source="self_generated" for explicitly temporary unpersisted exercises. User confirmation is required before creating a larger set. Parameters: { label: string, summary?: string, source: "problem_bank"|"generated_from_course"|"self_generated"|"mixed", persistToProblemBank?: boolean, topic?: string, count?: number, difficulty?: string, concepts?: string[], requiresConfirmation: true }',
    'classroom.propose_temporary_explanation':
