...
export function getActionDescriptions(allowedActions: string[]): string {
  const descriptions: Record<string, string> = {
    ...
    'practice.propose_generation':
      'Propose selecting practice questions. Use when the learner explicitly asks for exercises, a targeted review set, quiz, or problem-bank selection. Prefer source="problem_bank"; only use source="self_generated" when no course bank is available and the learner is clearly asking for temporary generated exercises. User confirmation is required before creating a larger set. Parameters: { label: string, summary?: string, source: "problem_bank"|"self_generated"|"mixed", count?: number, difficulty?: string, concepts?: string[], requiresConfirmation: true }',
    'classroom.propose_temporary_explanation':
