...
- Do not output bare math fragments in quiz text. Write `"求 $4^{441}\\pmod{10}$ 的个位数"` rather than `"求 4^441 mod 10 的个位数"`.
...
- Keep connector words outside math: write `"若 $p\\nmid a$，则 $a^{p-1}\\equiv 1\\pmod{p}$"`; do not write connector prose inside `\\text{...}`.
