...
- **Programming / algorithms**:
  ...
  - For ordinary CS lecture pages that introduce an object, compare representations, or summarize a rule, prefer fixed 16:9 classic layouts: `pipeline_table` for object/field/process/comparison pages, `text_image_split` / `two_text_image` when a real diagram belongs beside compact explanation, `four_columns` / `grid_2x2` for four parallel concepts, and `two_by_one_summary` for final recap pages.
  ...
  - `description` and `keyPoints` must be student-facing classroom content. Do not write lesson-plan prose such as "introduce the motivation", "establish the lesson spine", "this page is used to", or "emphasize the importance of". Phrase it as what the student sees or should try.
...
- **Proof-heavy mathematics**:
  - Plan the lesson spine as: concrete entry / motivation -> definition boundary -> worked proof or example -> practice check -> synthesis. Do not start with a vague roadmap or concept-card overview.
  - For function, set, image/preimage, injective/surjective/bijective, and similar proof topics, make the first two teaching pages introduce the object and define the test before any theorem chaining.
  ...
  - Show proof steps in order and explain why each step is valid; use `layoutFamily: "derivation"` with `layoutTemplate: "derivation_ladder"` or `problem_walkthrough` for proof walkthroughs, and make the page objective require "given/goal -> definition expansion -> connected proof moves -> next check".
  - Use `definition_board` / `formula_focus` for definition boundaries, `comparison_matrix` for comparing definition tests, and avoid generic 2x2 cards when a proof route is needed.
  - Call out common proof-writing mistakes such as missing justification, using the conclusion as a premise, confusing example with proof, or circular reasoning.
