...
const OUTPUT_DIR = path.resolve(process.cwd(), `public/generated-notebooks/${NOTEBOOK_ID}`);
const PUBLIC_DIR = `/generated-notebooks/${NOTEBOOK_ID}`;
const NOW = new Date();
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        speech:
          '把左端点的函数值代入：四段左黎曼和等于二乘以这四个左端点处函数值的总和。',
      },
      {
        id: 'numeric-answer',
        label: '数值答案',
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        speech: '总结成不等式：递增时，左端点和不超过真实面积，真实面积不超过右端点和；递减时顺序相反。',
      },
    ],
  },
  {
...
const parentRegionStepsByTitle = {
  'MAT 136 · 黎曼积分': [
    ...
    },
    {
      ...
      speech:
        '整节课的路线是：先讲黎曼和，再讲可积性，最后把下一节的定积分接上。',
    },
  ],
  '黎曼积分：从矩形到极限': [
    {
...
const imageAwareNarrationStepsByTitle = {
  ...
  '第二步：选择采样点 c_i': [
    {
      ...
      rect: [45, 35, 900, 120],
      speech:
        '上一页已经把底边切好了，也就是宽度有了。现在还差矩形的另一个部分：高度。',
    },
    {
    ...
    },
    {
      ...
      speech:
        '右边是中点规则。采样点放在小区间正中间，用中间位置的函数值作为矩形高度。',
    },
    {
      id: 'area-rule',
      label: '面积公式框',
    ...
    },
    {
      ...
      rect: [270, 780, 980, 90],
      speech:
        '所以第二步的核心是：采样点决定矩形的高度。下一页就把每个小矩形的面积加起来。',
    },
  ],
  '第三步：把矩形面积加起来': [
    {
    ...
    },
    {
      ...
      speech:
        '右下角再把所有矩形相加。求和符号不用怕，它只是把第一段、第二段，一直加到最后一段。',
    },
    {
      id: 'riemann-sum-name',
      label: '底部名称',
      rect: [340, 785, 860, 80],
      speech:
        '这个总和就叫黎曼和。下一页我们用一个具体数字例题，看看左端点规则到底怎么计算。',
    },
  ],
  '例题 1：左黎曼和估计 √x': [
    {
    ...
    },
    {
      ...
      rect: [65, 225, 735, 470],
      speech:
        '左边图像展示四段等分。总长度是四十减三十二，也就是八；分成四段，所以每段宽度是二。',
    },
    {
  ...
  '例题 2：只有表格，也能算右黎曼和': [
    ...
    },
    {
      ...
      rect: [55, 235, 380, 260],
      speech:
        '先读表格。横坐标的位置是零、二、四、六、八，对应的函数值分别是三、五、四、七、六。',
    },
    {
      id: 'graph-panel',
      label: '右端点矩形区',
    ...
    },
    {
      ...
      speech:
        '右上角把两个关键信息单独列出来：每段宽度是二，右端点是二、四、六、八。',
    },
    {
      id: 'calculation-panel',
      label: '右黎曼和计算区',
  ...
  三种常见采样规则: [
    ...
    },
    {
      ...
      rect: [30, 185, 500, 530],
      speech:
        '左边是左端点和。每个小矩形都用左边界的高度，所以这些采样点都贴在每段的左侧。',
    },
    {
      id: 'right-rule-panel',
      label: '右端点和面板',
      ...
      speech:
        '中间是右端点和。分割可以和左边完全一样，但高度改成每段右边界的函数值。',
    },
    {
      ...
      speech:
        '右边是中点和。每个采样点放在小区间中间，所以高度来自中点位置。',
    },
    {
      id: 'comparison-summary',
      label: '底部比较',
  ...
  '高估 / 低估：先看单调性': [
    ...
    },
    {
      ...
      rect: [60, 475, 720, 260],
      speech:
        '左下角换成递减函数。现在左端点反而是每段较高的位置，所以左端点矩形会偏高。',
    },
    {
      id: 'decreasing-right',
      label: '递减右端点低估',
      ...
      speech:
        '右下角递减加右端点。右端点在每段较低的位置，所以矩形落在曲线下面，是低估。',
    },
    {
      id: 'inequality-summary',
      label: '底部不等式',
  ...
  '让分割变细：mesh 趋近 0': [
    {
      ...
      rect: [45, 35, 900, 120],
      speech:
        '上一页的高估和低估都来自矩形跟曲线之间的缝隙。现在我们做一个自然动作：把分割变细。',
    },
    {
      id: 'coarse-partition',
      label: '粗分割面板',
      rect: [40, 165, 705, 430],
      speech:
        '左边是粗分割。矩形很宽，最大区间宽度也很大，所以曲线和矩形之间的误差比较明显。',
    },
    {
      id: 'refine-arrow',
      label: 'refine 箭头',
    ...
    },
    {
      ...
      speech:
        '右边是细分割。矩形变窄以后，矩形顶边更贴近曲线，最大区间宽度也变小了。',
    },
    {
      id: 'mesh-definition',
      label: 'mesh 定义框',
  ...
  '什么时候叫“可黎曼积分”？': [
    {
      ...
      rect: [45, 35, 900, 120],
      speech:
        '上一页问：分割越来越细时，所有合法采样会不会走向同一个数？这一页就给出答案。',
    },
    {
      id: 'any-sample-point',
      label: '任意采样点',
  ...
  '本节课总结：黎曼积分的流程': [
    {
      ...
      rect: [45, 35, 1060, 120],
      speech:
        '上一页给出了可黎曼积分的判断。最后这一页把整条流程收回来，确认每一步分别在做什么。',
    },
    {
      id: 'five-step-flow',
      label: '顶部五步流程',
...
for (const slide of slides) {
  const override = imageAwareNarrationStepsByTitle[slide.title] ?? parentRegionStepsByTitle[slide.title];
  if (!override) {
