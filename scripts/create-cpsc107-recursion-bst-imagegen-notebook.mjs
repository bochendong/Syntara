...
async function copySlides() {
  ...
  fs.writeFileSync(path.join(OUTPUT_DIR, 'selected-image-sources.json'), JSON.stringify(copied, null, 2));
}
...
function writeMetadata() {
  ...
  fs.writeFileSync(path.join(OUTPUT_DIR, 'semantic-hit-map.json'), JSON.stringify(semanticHitMap, null, 2));
  fs.writeFileSync(path.join(OUTPUT_DIR, 'scene-actions.json'), JSON.stringify(scenes.map((scene) => ({
    id: scene.id,
    title: scene.title,
    order: scene.order,
    ...
    JSON.stringify(
      {
        ...
        description: '从阶乘和 Fibonacci 建立递归直觉，再进入 list recursion、helper function 和 binary search tree lookup。',
        courseId: COURSE_ID,
  ...
  try {
    ...
    await prisma.notebook.upsert({
      ...
      update: {
        ...
        description: '从阶乘和 Fibonacci 建立递归直觉，再进入 list recursion、helper function 和 BST lookup。',
