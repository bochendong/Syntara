...
const WIDTH = 1600;
const HEIGHT = 900;
const DATA_PATH = path.resolve(process.cwd(), 'scripts/mat102-queue-zh-notebooks.json');
...
function parseArgs(argv) {
  ...
  for (const arg of argv.slice(2)) {
    if (arg === '--force') options.force = true;
      ...
      options.only = new Set(arg.slice('--only='.length).split(',').map((item) => item.trim()));
    }
  }
  return options;
}
...
function outputDir(slug) {
  return path.resolve(process.cwd(), 'public/generated-notebooks', notebookId(slug));
}
  ...
  return items
    .map((item, index) => {
      ...
      return [
        ...
        textBlock(wrapText(item, 22, 2), x + 34, top, { size, lineHeight: 1.18, weight: 600, fill: '#334155' }),
      ].join('');
    })
    .join('');
}
...
function getKeywords(title, intent, slug) {
  ...
  if (slug.includes('functions-ii')) {
    ...
    if (/基数|count|无限|Cantor|Schroder|对角线/.test(source)) return ['|S| <= |T|', 'N ~ 2N', 'diagonal'];
  ...
  if (slug.includes('number-theory-ii')) {
    if (/丢番图|整数解|方程|所有解/.test(source)) return ['ax + by = c', 'gcd(a,b) | c', 'x = x0 + bt'];
    if (/素数|算术基本|无限/.test(source)) return ['p | ab', 'prime factorization', 'N = p1...pk + 1'];
...
function functionsDiagram(keyword, theme) {
  ...
  return [
    ...
    ...arrows.map(([x1, y1, x2, y2]) => `<path d="M ${x1} ${y1} C ${x1 + 95} ${y1}, ${x2 - 95} ${y2}, ${x2} ${y2}" fill="none" stroke="${theme.accent}" stroke-width="6" stroke-linecap="round" marker-end="url(#arrow-${theme.name})"/>`),
    ...nodesLeft.map(([label, x, y]) => `<circle cx="${x}" cy="${y}" r="38" fill="${theme.soft}" stroke="${theme.accent}" stroke-width="4"/><text x="${x}" y="${y + 11}" text-anchor="middle" fill="#0f172a" font-size="34" font-weight="800" font-family="Arial">${label}</text>`),
    ...nodesRight.map(([label, x, y]) => `<circle cx="${x}" cy="${y}" r="38" fill="#ecfdf5" stroke="${theme.secondary}" stroke-width="4"/><text x="${x}" y="${y + 11}" text-anchor="middle" fill="#0f172a" font-size="34" font-weight="800" font-family="Arial">${label}</text>`),
  ].join('');
}
...
function slideSvg(notebook, order) {
  ...
  const section = order === 0 ? '课程封面' : order < 3 ? '引入与问题' : order < total - 2 ? '核心推进' : '总结与连接';
...
async function renderNotebook(notebook, options) {
  ...
  for (const [order] of notebook.slides.entries()) {
    ...
    await sharp(Buffer.from(slideSvg(notebook, order))).png().toFile(destination);
  }
  fs.writeFileSync(
