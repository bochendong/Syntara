...
const OUTPUT_ROOT = path.resolve(process.cwd(), 'public/generated-notebooks');
...
function labelFor(element) {
  ...
  return String(raw).replace(/^semantic-hit-map:\s*/, '').trim();
}
...
function stripGenerated(elements) {
  return elements.filter((element) => !String(element?.id || '').includes('lecture-focus-generated'));
}
...
function focusElementsForScene(spec, notebookId, scene) {
  ...
  if (!shapeElements.length) {
    ...
    return { elements: [...original, ...generated], focuses: generated, generatedCount: generated.length };
  }
...
function explanationForKind(kind, scene, focus, spec, isLastFocus) {
  ...
  switch (kind) {
    case 'cover':
      ...
      break;
    case 'title':
      lines.push(`先看标题区。这一页叫《${title}》，它在本节主线里的任务是：${mainMoveForScene(scene)}`);
      lines.push(`本节主线先放在心里：${spec.logline}`);
      break;
    case 'purpose':
      lines.push('先看 purpose。它不是一句注释装饰，而是在 body 出现之前，先承诺这个函数到底要做什么。');
    ...
    case 'body':
      ...
      break;
    case 'data':
      lines.push(`现在看 ${label || '数据定义'}。先别写函数，先读这个数据有哪些可能形状、哪些字段。`);
      lines.push('字段类型决定后面的模板动作：原子值直接用，compound 要 selector，别的 HTDD 要 helper。');
      break;
    case 'branch':
      ...
      break;
    case 'selector':
      lines.push('现在看 selector。compound data 不能凭感觉取字段，必须用对应 selector 把字段拆出来。');
    ...
    case 'recursive':
      ...
      break;
    case 'helper':
      lines.push('现在看 helper 或引用位置。字段如果是另一个 HTDD，就不要在这里硬展开，交给对应 helper。');
    ...
    case 'codeBranch':
      ...
      break;
    case 'codeRecursive':
      lines.push('这一小块看递归调用、helper call 或返回值。这里决定当前层怎样把子问题答案接回来。');
...
function validateScene(scene) {
  ...
  const ids = new Set();
  const distant = /学生|讲的时候|让学生|学习者|听众|用户|课堂停顿|帮学生|学生应该|学生会|可以让学生/;
  ...
  for (const action of scene.actions || []) {
    ...
    if (badSymbols.test(text)) throw new Error(`${scene.title}: TTS-unfriendly symbol in "${text}"`);
  ...
  try {
    ...
    for (const [notebookId, spec] of Object.entries(NOTEBOOK_SPECS)) {
      ...
      const updatedScenes = notebook.scenes.map((scene) => {
        const { elements, focuses, generatedCount } = focusElementsForScene(spec, notebookId, scene);
