...
function workedBoardLeadFor(title) {
  ...
  if (/^例题/.test(title)) return '现在看右侧例题。先读题目给了什么，再判断要调用哪一个定义或定理。';
  if (/常见错误/.test(title)) return '现在看右侧错误提醒。不要只记错在哪里，要说出是哪条定义或条件被破坏了。';
...
function writeArtifacts(notebookId, scenes) {
  const outputDir = path.resolve(process.cwd(), 'public/generated-notebooks', notebookId);
